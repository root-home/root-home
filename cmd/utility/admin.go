...
func admin() {
	...
	init := flag.Bool(
		"init",
		false,
		"Inits a new goHOME config and system file in the current directory. You must specify the path to the root of the goHOME source code as the only argument to --init Usage: ghadmin --init /path/to/rhome")
...
func initConfig(webUIPath string) *rhome.Config {
	...
	cfg := rhome.NewDefaultConfig(currentDir, webUIPath)
...
func initSystem(cfg *rhome.Config) {
	...
	sys := rhome.NewSystem("My goHOME system")
...
func loadSystem(systemPath string) *rhome.System {
