...
import (
	...
	"github.com/root-home/event-bus"
	"github.com/root-home/upnp"
	"github.com/root-home/root-home/pkg/clock"
	"github.com/root-home/root-home/pkg/rhome"
	"github.com/root-home/root-home/pkg/log"
	"github.com/root-home/root-home/pkg/store"
	"github.com/root-home/root-home/pkg/www"
)
...
func server() {
	...
	cp := rhome.NewCommandProcessor(sys, 3, 1000)
	cp.Start()
	...
	monitor := rhome.NewMonitor(sys, sys.Services.EvtBus)
	sys.Services.Monitor = monitor

	// Log all of the events on the bus to the event log
	evtLogger := &rhome.EventLogger{Path: cfg.EventLogPath, Verbose: false}
	...
	autos, err := rhome.LoadAutomation(sys, cfg.AutomationPath)
	if err != nil {
	...
	for _, auto := range autos {
		...
		auto.Triggered = func(actions *rhome.CommandGroup) {
			sys.Services.EvtBus.Enqueue(&rhome.AutomationTriggeredEvt{
				Name: auto.Name,
			})
	...
	sys.Services.EvtBus.Enqueue(&rhome.ServerStartedEvt{})
...
func loadSystem(configPath string) (*rhome.System, rhome.Config) {
